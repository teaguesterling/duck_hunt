...
static std::string MapEventNameToSeverity(const std::string &event_name, bool has_error) {
	...
	if (event_name.find("Delete") != std::string::npos || event_name.find("Terminate") != std::string::npos ||
	    event_name.find("Modify") != std::string::npos || event_name.find("Update") != std::string::npos ||
...
static bool ParseCloudTrailRecord(const std::string &record, ValidationEvent &event, int64_t event_id,
                                  int line_number) {
	// Extract key fields
