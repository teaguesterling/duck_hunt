...
std::vector<ValidationEvent> LcovParser::parse(const std::string& content) const {
    ...
    if (content.empty()) return events;
    ...
    auto emit_file_events = [&]() {
        ...
        double coverage_pct = lines_found > 0 ?
            (static_cast<double>(lines_hit) / lines_found * 100.0) : 0.0;
        ...
        summary.message = std::to_string(lines_hit) + "/" + std::to_string(lines_found) +
                         " lines covered (" + std::to_string(coverage_pct) + "%)";
