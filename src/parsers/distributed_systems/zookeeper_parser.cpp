...
static bool ParseZookeeperLine(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	// Expect " - " separator
	if (pos + 3 > line.size() || line.substr(pos, 3) != " - ")
		return false;
	pos += 3;
...
bool ZookeeperParser::canParse(const std::string &content) const {
	...
	while (reader.getLine(line) && checked < 10) {
		...
		if (line.size() >= 30) {
			...
			if (is_zk && line[4] != '-')
				is_zk = false;
			if (is_zk && (!std::isdigit(line[5]) || !std::isdigit(line[6])))
				is_zk = false;
			if (is_zk && line[7] != '-')
				is_zk = false;
			if (is_zk && (!std::isdigit(line[8]) || !std::isdigit(line[9])))
				is_zk = false;

			// Check for " - " separator after timestamp and level in brackets
			if (is_zk) {
				// Look for " - INFO  [" or " - WARN  [" etc.
				if ((line.find(" - INFO", 20) != std::string::npos || line.find(" - WARN", 20) != std::string::npos ||
