...
static bool ParseHdfsLine(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	if (pos == comp_start || pos >= line.size())
		return false;
...
bool HdfsParser::canParse(const std::string &content) const {
	...
	while (reader.getLine(line) && checked < 10) {
		...
		if (line.size() >= 20) {
			...
			if (is_hdfs) {
				if (line.find(" INFO ", 14) != std::string::npos || line.find(" WARN ", 14) != std::string::npos ||
				    line.find(" ERROR ", 14) != std::string::npos || line.find(" DEBUG ", 14) != std::string::npos ||
