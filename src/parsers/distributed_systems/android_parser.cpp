...
static bool ParseAndroidLine(const std::string& line, ValidationEvent& event, int64_t event_id, int line_number) {
    ...
    // Date: MM-DD
    if (!std::isdigit(line[pos]) || !std::isdigit(line[pos+1])) return false;
    ...
    // Time: HH:MM:SS.mmm
    if (pos + 12 > line.size()) return false;
    ...
    pos++;
    // mmm (milliseconds)
    if (!std::isdigit(line[pos]) || !std::isdigit(line[pos+1]) || !std::isdigit(line[pos+2])) return false;
    ...
    char level = line[pos];
    if (level != 'V' && level != 'D' && level != 'I' && level != 'W' &&
        level != 'E' && level != 'F' && level != 'S') {
        return false;
    }
    pos++;
    ...
    if (pos == tag_start || pos >= line.size()) return false;
    ...
    event.structured_data = "{\"tag\":\"" + tag + "\",\"level\":\"" + std::string(1, level) +
                           "\",\"pid\":\"" + pid + "\",\"tid\":\"" + tid + "\"}";
...
bool AndroidParser::canParse(const std::string& content) const {
    ...
    while (reader.getLine(line) && checked < 10) {
        ...
        if (line.size() >= 25) {
            ...
            // Check MM-DD pattern
            if (!std::isdigit(line[0]) || !std::isdigit(line[1])) is_android = false;
            if (is_android && line[2] != '-') is_android = false;
            if (is_android && (!std::isdigit(line[3]) || !std::isdigit(line[4]))) is_android = false;
            if (is_android && line[5] != ' ') is_android = false;

            // Check HH:MM:SS.mmm pattern
            if (is_android && (!std::isdigit(line[6]) || !std::isdigit(line[7]))) is_android = false;
            if (is_android && line[8] != ':') is_android = false;
            if (is_android && (!std::isdigit(line[9]) || !std::isdigit(line[10]))) is_android = false;
            if (is_android && line[11] != ':') is_android = false;
            if (is_android && (!std::isdigit(line[12]) || !std::isdigit(line[13]))) is_android = false;
            if (is_android && line[14] != '.') is_android = false;
            ...
            if (is_android) {
                ...
                if (level_pos == std::string::npos) level_pos = line.find(" D ", 18);
                if (level_pos == std::string::npos) level_pos = line.find(" I ", 18);
                if (level_pos == std::string::npos) level_pos = line.find(" W ", 18);
                if (level_pos == std::string::npos) level_pos = line.find(" E ", 18);
                if (level_pos == std::string::npos) level_pos = line.find(" F ", 18);
