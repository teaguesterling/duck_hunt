...
static bool ParseSparkLine(const std::string& line, ValidationEvent& event, int64_t event_id, int line_number) {
    ...
    if (!std::isdigit(line[pos+3]) || !std::isdigit(line[pos+4])) return false;
    if (line[pos+5] != ':') return false;
    if (!std::isdigit(line[pos+6]) || !std::isdigit(line[pos+7])) return false;
...
bool SparkParser::canParse(const std::string& content) const {
    ...
    while (reader.getLine(line) && checked < 10) {
        ...
        if (line.size() >= 20) {
            ...
            if (!std::isdigit(line[0]) || !std::isdigit(line[1])) is_spark = false;
            if (is_spark && line[2] != '/') is_spark = false;
            if (is_spark && (!std::isdigit(line[3]) || !std::isdigit(line[4]))) is_spark = false;
            if (is_spark && line[5] != '/') is_spark = false;
            if (is_spark && (!std::isdigit(line[6]) || !std::isdigit(line[7]))) is_spark = false;
            if (is_spark && line[8] != ' ') is_spark = false;

            // Check for INFO/WARN/ERROR after timestamp
            if (is_spark) {
                if (line.find(" INFO ", 17) != std::string::npos ||
                    line.find(" WARN ", 17) != std::string::npos ||
                    line.find(" ERROR ", 17) != std::string::npos ||
                    line.find(" DEBUG ", 17) != std::string::npos ||
