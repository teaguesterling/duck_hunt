...
static bool ParseOpenStackLine(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	for (size_t i = 0; i + 23 < line.size(); i++) {
		if (std::isdigit(line[i]) && std::isdigit(line[i + 1]) && std::isdigit(line[i + 2]) &&
		    std::isdigit(line[i + 3]) && line[i + 4] == '-' && std::isdigit(line[i + 5]) && std::isdigit(line[i + 6]) &&
		    line[i + 7] == '-' && std::isdigit(line[i + 8]) && std::isdigit(line[i + 9]) && line[i + 10] == ' ' &&
		    std::isdigit(line[i + 11]) && std::isdigit(line[i + 12]) && line[i + 13] == ':') {
			if (i > 0) {
	...
	// Validate level
	if (level != "INFO" && level != "WARNING" && level != "WARN" && level != "ERROR" && level != "CRITICAL" &&
	    level != "DEBUG" && level != "TRACE" && level != "AUDIT" && level != "FATAL") {
		return false;
	}
...
bool OpenStackParser::canParse(const std::string &content) const {
	...
	while (reader.getLine(line) && checked < 10) {
		...
		bool has_openstack_component =
		    (line.find("nova.") != std::string::npos || line.find("neutron.") != std::string::npos ||
		     line.find("cinder.") != std::string::npos || line.find("glance.") != std::string::npos ||
		     line.find("keystone.") != std::string::npos || line.find("swift.") != std::string::npos ||
