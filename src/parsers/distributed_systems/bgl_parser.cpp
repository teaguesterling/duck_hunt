...
static bool ParseBglLine(const std::string& line, ValidationEvent& event, int64_t event_id, int line_number) {
    ...
    if (level != "INFO" && level != "FATAL" && level != "WARNING" &&
        level != "FAILURE" && level != "ERROR" && level != "SEVERE" &&
        level != "DEBUG") {
        return false;
    }
    ...
    event.structured_data = "{\"node\":\"" + node + "\",\"source\":\"" + source +
...
bool BglParser::canParse(const std::string& content) const {
    ...
    while (reader.getLine(line) && checked < 10) {
        ...
        if (line.size() >= 2 && line[0] == '-' && line[1] == ' ') {
            ...
            if (line.compare(0, 4, "APP ") == 0 ||
                line.compare(0, 7, "APPREAD") == 0 ||
                line.compare(0, 8, "APPERROR") == 0 ||
                line.compare(0, 7, "KERNELP") == 0) {
                starts_correctly = true;
            }
        }
        ...
        for (size_t i = 0; i + 10 < line.size(); i++) {
            if (line[i] == 'R' && std::isdigit(line[i+1]) && std::isdigit(line[i+2]) &&
                line[i+3] == '-' && line[i+4] == 'M') {
                has_node = true;
                break;
            }
