...
// 1. auditd format:
//    "type=SYSCALL msg=audit(1610721045.123:456): arch=c000003e syscall=59 success=yes exit=0 ... comm=\"sudo\"
//    exe=\"/usr/bin/sudo\" key=\"privileged\""
//
...
static std::unordered_map<std::string, std::string> ParseAuditFields(const std::string &line) {
	...
	static std::regex kv_pattern(R"(([a-zA-Z_][a-zA-Z0-9_]*)=(?:\"([^\"]*)\"|([^\s]*)))", std::regex::optimize);
...
static bool ParseAuditdLine(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	static std::regex audit_pattern(R"(type=(\S+)\s+msg=audit\((\d+\.\d+):(\d+)\):\s*(.*))", std::regex::optimize);
	...
	if (std::regex_search(line, match, audit_pattern)) {
		...
		if (audit_type.find("AVC") != std::string::npos || audit_type.find("SELINUX") != std::string::npos ||
		    success == "no") {
		...
		if (fields.count("comm"))
			msg += " comm=" + fields["comm"];
		if (fields.count("exe"))
			msg += " exe=" + fields["exe"];
		if (fields.count("key"))
			msg += " key=" + fields["key"];
	...
	static std::regex ssh_pattern(R"(^(\w{3}\s+\d+\s+\d{2}:\d{2}:\d{2})\s+(\S+)\s+sshd\[(\d+)\]:\s*(.*)$)",
	                              std::regex::optimize);
	...
	if (std::regex_match(line, match, ssh_pattern)) {
		...
		if (message.find("Failed") != std::string::npos || message.find("Invalid") != std::string::npos ||
		    message.find("error") != std::string::npos || message.find("refused") != std::string::npos) {
			event.severity = "warning";
			event.status = ValidationEventStatus::WARNING;
		...
		if (!event.origin.empty())
			json += ",\"source_ip\":\"" + event.origin + "\"";
		if (!event.principal.empty())
			json += ",\"user\":\"" + event.principal + "\"";
		json += "}";
		event.structured_data = json;
