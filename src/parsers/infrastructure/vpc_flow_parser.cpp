...
static std::string ProtocolName(const std::string& proto_num) {
    ...
    if (proto_num == "47") return "GRE";
    if (proto_num == "50") return "ESP";
    if (proto_num == "51") return "AH";
    if (proto_num == "58") return "ICMPv6";
    return proto_num;
}
...
static bool ParseVpcFlowLine(const std::string& line, ValidationEvent& event, int64_t event_id, int line_number,
    ...
    if (fields[0] == "version") return false; // Header line
    ...
    if (!srcaddr.empty()) {
        msg += srcaddr;
        if (!srcport.empty() && srcport != "0") msg += ":" + srcport;
    }
    msg += " -> ";
    if (!dstaddr.empty()) {
        msg += dstaddr;
        if (!dstport.empty() && dstport != "0") msg += ":" + dstport;
    }
    msg += " (" + proto_name + ")";
    ...
    if (!account_id.empty()) json += ",\"account_id\":\"" + account_id + "\"";
    if (!interface_id.empty()) json += ",\"interface_id\":\"" + interface_id + "\"";
    if (!srcaddr.empty()) json += ",\"srcaddr\":\"" + srcaddr + "\"";
    if (!dstaddr.empty()) json += ",\"dstaddr\":\"" + dstaddr + "\"";
    if (!srcport.empty()) json += ",\"srcport\":\"" + srcport + "\"";
    if (!dstport.empty()) json += ",\"dstport\":\"" + dstport + "\"";
    if (!protocol.empty()) json += ",\"protocol\":\"" + proto_name + "\"";
    if (!packets.empty() && packets != "-") json += ",\"packets\":\"" + packets + "\"";
    if (!bytes.empty() && bytes != "-") json += ",\"bytes\":\"" + bytes + "\"";
    if (!action.empty()) json += ",\"action\":\"" + action + "\"";
    if (!log_status.empty()) json += ",\"log_status\":\"" + log_status + "\"";
    json += "}";
