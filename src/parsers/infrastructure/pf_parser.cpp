...
static bool ParsePfLine(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	static std::regex filterlog_pattern(R"(filterlog:\s*(\d+),)", std::regex::optimize);
	...
	if (std::regex_search(line, match, pf_pattern)) {
		...
		event.message = action + " " + direction + ": " + src_ip + ":" + src_port + " -> " + dst_ip + ":" + dst_port;
		if (!flags.empty()) {
			event.message += " [" + flags + "]";
		}
...
bool PfParser::canParse(const std::string &content) const {
	...
	static std::regex pf_detect(R"(rule\s+\d+/\d+\(\w+\):\s+(pass|block|match))",
	                            std::regex::optimize | std::regex::icase);
