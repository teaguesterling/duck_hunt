...
//
// Example:
// 79a59df900b949e55d96a1e698fbacedfd6e09d98eacf8f8d5218e7cd47ef2be mybucket [15/Jan/2025:10:30:45 +0000] 192.168.1.100 arn:aws:iam::123456789012:user/username ABCDEF123456 REST.GET.OBJECT mykey.txt "GET /mybucket/mykey.txt HTTP/1.1" 200 - 12345 12345 50 49 "-" "aws-cli/2.0.0" - abc123= SigV4 ECDHE-RSA-AES128-SHA AuthHeader mybucket.s3.amazonaws.com TLSv1.2
...
static bool ParseS3AccessLine(const std::string& line, ValidationEvent& event, int64_t event_id, int line_number) {
    ...
    std::string json = "{";
    if (!bucket_owner.empty()) json += "\"bucket_owner\":\"" + bucket_owner + "\"";
    if (!bucket.empty()) {
