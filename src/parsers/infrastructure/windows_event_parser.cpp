...
static bool ParseEventRecord(const WindowsEventRecord& record, ValidationEvent& event, int64_t event_id) {
    ...
    std::string json = "{";
    if (!record.log_name.empty()) json += "\"log_name\":\"" + record.log_name + "\"";
...
std::vector<ValidationEvent> WindowsEventParser::parse(const std::string& content) const {
    ...
    while (std::getline(stream, line)) {
        ...
        if (trimmed.find("Source:") == 0) {
            ...
            if (start != std::string::npos) current_record.source = current_record.source.substr(start);
        } else if (trimmed.find("Date:") == 0) {
            in_description = false;
            current_record.date = trimmed.substr(5);
            size_t start = current_record.date.find_first_not_of(" \t");
            if (start != std::string::npos) current_record.date = current_record.date.substr(start);
            ...
            if (start != std::string::npos) current_record.event_id = current_record.event_id.substr(start);
            ...
            if (start != std::string::npos) current_record.task_category = current_record.task_category.substr(start);
        } else if (trimmed.find("Level:") == 0) {
            in_description = false;
            current_record.level = trimmed.substr(6);
            size_t start = current_record.level.find_first_not_of(" \t");
            if (start != std::string::npos) current_record.level = current_record.level.substr(start);
            ...
            if (start != std::string::npos) current_record.keywords = current_record.keywords.substr(start);
        } else if (trimmed.find("User:") == 0) {
            in_description = false;
            current_record.user = trimmed.substr(5);
            size_t start = current_record.user.find_first_not_of(" \t");
            if (start != std::string::npos) current_record.user = current_record.user.substr(start);
            ...
            if (start != std::string::npos) current_record.computer = current_record.computer.substr(start);
