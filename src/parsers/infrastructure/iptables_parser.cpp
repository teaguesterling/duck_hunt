...
// "Jan 15 10:30:45 hostname kernel: [12345.678901] IN=eth0 OUT= MAC=... SRC=192.168.1.100 DST=10.0.0.1 ... PROTO=TCP
// SPT=54321 DPT=22 ..." Also handles UFW format: "Jan 15 10:30:45 hostname kernel: [UFW BLOCK] IN=eth0 OUT= MAC=...
// SRC=..."
...
static bool ParseIptablesLine(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	if (line.find("kernel:") == std::string::npos && line.find("kernel[") == std::string::npos) {
		return false;
	}
	...
	if (!dpt.empty())
		json += ",\"dst_port\":\"" + dpt + "\"";
	if (!in_iface.empty())
		json += ",\"in_interface\":\"" + in_iface + "\"";
	if (!out_iface.empty())
		json += ",\"out_interface\":\"" + out_iface + "\"";
	if (fields.count("MAC"))
		json += ",\"mac\":\"" + fields["MAC"] + "\"";
	if (fields.count("TTL"))
		json += ",\"ttl\":\"" + fields["TTL"] + "\"";
	if (fields.count("LEN"))
		json += ",\"length\":\"" + fields["LEN"] + "\"";
	json += "}";
...
bool IptablesParser::canParse(const std::string &content) const {
	...
	while (std::getline(stream, line) && checked < 10) {
		...
		bool has_kernel = (line.find("kernel:") != std::string::npos || line.find("kernel[") != std::string::npos);
		bool has_fields = (line.find("SRC=") != std::string::npos || line.find("IN=") != std::string::npos);
