...
static std::string MapAsaSeverity(int level) {
	// ASA levels: 0=emergencies, 1=alerts, 2=critical, 3=errors, 4=warnings, 5=notifications, 6=informational,
	// 7=debugging
	if (level <= 3) {
		return "error";
	} else if (level == 4) {
...
// "Jan 15 2025 10:30:45 hostname : %ASA-6-302013: Built inbound TCP connection 12345 for outside:192.168.1.100/54321 to
// inside:10.0.0.1/22" Also handles:
...
static bool ParseCiscoAsaLine(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	static std::regex asa_pattern(R"(%ASA-(\d)-(\d+):\s*(.*))", std::regex::optimize);
	...
	static std::regex src_pattern(R"((?:src|from)\s+\w+:(\d+\.\d+\.\d+\.\d+)/(\d+))",
	                              std::regex::optimize | std::regex::icase);
	static std::regex dst_pattern(R"((?:dst|to)\s+\w+:(\d+\.\d+\.\d+\.\d+)/(\d+))",
	                              std::regex::optimize | std::regex::icase);
	...
	if (message_text.find("Deny") != std::string::npos || message_text.find("deny") != std::string::npos) {
		...
		action = "teardown";
	} else if (message_text.find("Permitted") != std::string::npos ||
	           message_text.find("permitted") != std::string::npos) {
		action = "permit";
	}
