    ...
    while (std::getline(iss, line)) {
        if (isPhaseMarker(line)) {
            ...
            if (!current_phase_lines.empty() && !current_phase.empty()) {
                current_build.phases.push_back(
                    parsePhase(current_phase_lines, current_package, current_phase));
    ...
    for (const auto& build : builds) {
        for (const auto& phase : build.phases) {
            for (const auto& output_line : phase.output_lines) {
                ...
                ValidationEvent base_event = createBaseEvent(output_line,
                    "Spack Build: " + build.package_name,
                    phase.phase_name,
