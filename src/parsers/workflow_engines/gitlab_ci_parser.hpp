...
private:
	struct GitLabStage {
		...
		// Delegation support
		std::string detected_command;                  // Command extracted from script execution
	...
	std::vector<WorkflowEvent> convertToEvents(const std::vector<GitLabJob> &jobs, const std::string &workflow_name,
