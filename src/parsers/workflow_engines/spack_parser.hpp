...
public:
    ...
    std::string getFormatName() const override { return "spack"; }
    std::vector<WorkflowEvent> parseWorkflowLog(const std::string& content) const override;
    int getPriority() const override { return 140; } // High priority for Spack
    std::string getName() const override { return "SpackParser"; }
...
private:
    ...
    SpackPhase parsePhase(const std::vector<std::string>& phase_lines,
