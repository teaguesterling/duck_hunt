...
static bool ParseAccessLogLine(const std::string& line, ValidationEvent& event, int64_t event_id, int line_number, const std::string& tool_name) {
    // Combined Log Format regex:
    // IP - user [timestamp] "method path protocol" status size "referrer" "user-agent"
    static std::regex combined_pattern(
        R"(^(\S+)\s+\S+\s+\S+\s+\[([^\]]+)\]\s+\"(\S+)\s+(\S+)\s+([^\"]*)\"\s+(\d+)\s+(\d+|-)(?:\s+\"([^\"]*)\"\s+\"([^\"]*)\")?)",
        std::regex::optimize
    );

    std::smatch match;
    if (!std::regex_match(line, match, combined_pattern)) {
        return false;
    }
...
bool ApacheAccessParser::canParse(const std::string& content) const {
    if (content.empty()) {
        return false;
    }
...
std::vector<ValidationEvent> ApacheAccessParser::parse(const std::string& content) const {
    ...
    while (std::getline(stream, line)) {
        ...
        if (ParseAccessLogLine(line, event, event_id, line_number, "apache_access")) {
