...
bool LogrusParser::canParse(const std::string &content) const {
	...
	static std::regex kv_detect_pattern(R"(time=|level=|msg=)", std::regex::optimize);
	static std::regex console_detect_pattern(R"(^(PANIC|FATAL|ERROR|WARN|WARNING|INFO|DEBUG|TRACE)\[\d+\])",
	                                         std::regex::optimize | std::regex::icase);
	...
	while (std::getline(stream, line) && checked < 10) {
		...
		if (std::regex_search(line, kv_detect_pattern) || std::regex_search(line, console_detect_pattern)) {
			logrus_lines++;
		}
	}
