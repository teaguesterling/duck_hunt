...
static bool ParsePythonLogLine(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	static std::regex simple_pattern(R"(^(DEBUG|INFO|WARNING|WARN|ERROR|CRITICAL|FATAL):(\S+):(.*)$)",
	                                 std::regex::optimize | std::regex::icase);
...
static bool IsTracebackLine(const std::string &line) {
	...
	// Code line (indented)
	if (line.size() >= 4 && line.substr(0, 4) == "    " && line.find("  File \"") == std::string::npos) {
		return true;
	}
...
bool PythonLoggingParser::canParse(const std::string &content) const {
	...
	static std::regex simple_level_pattern(R"(^(DEBUG|INFO|WARNING|WARN|ERROR|CRITICAL|FATAL):)",
	                                       std::regex::optimize | std::regex::icase);
	...
	while (std::getline(stream, line) && checked < 10) {
		...
		if (std::regex_search(line, python_dash_pattern) || std::regex_search(line, simple_level_pattern)) {
			python_log_lines++;
		}
	}
	...
	while (std::getline(stream, line)) {
		...
		if (IsTracebackLine(line)) {
			if (current_event != nullptr) {
				...
				static std::regex file_line_pattern(R"re(^\s*File\s+"([^"]+)",\s+line\s+(\d+)(?:,\s+in\s+(\S+))?)re",
				                                    std::regex::optimize);
				std::smatch match;
