...
static bool ParseSerilogText(const std::string &line, ValidationEvent &event, int64_t event_id, int line_number) {
	...
	static std::regex serilog_short_pattern(R"(^\[(\d{2}:\d{2}:\d{2})\s+(VRB|DBG|INF|WRN|ERR|FTL)\]\s*(.*)$)",
	                                        std::regex::optimize);
...
bool SerilogParser::canParse(const std::string &content) const {
	...
	static std::regex text_detect(
	    R"(^\[\d{2}:\d{2}:\d{2}\s+(VRB|DBG|INF|WRN|ERR|FTL)\]|^\[.*\]\s+\[(VRB|DBG|INF|WRN|ERR|FTL)\])",
	    std::regex::optimize);
