...
static bool ParseRailsLine(const std::string &line, RailsRequest &request, int line_number) {
	// Started line
	static std::regex started_pattern(R"(^Started\s+(\w+)\s+\"([^\"]+)\"\s+for\s+(\S+)\s+at\s+(.+)$)",
	                                  std::regex::optimize);

	// Processing line
	static std::regex processing_pattern(R"(^Processing\s+by\s+(\w+)#(\w+)\s+as\s+(\w+))", std::regex::optimize);
...
bool RailsLogParser::canParse(const std::string &content) const {
	...
	static std::regex started_detect(R"(^Started\s+(GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\s+\")",
	                                 std::regex::optimize);
	...
	while (std::getline(stream, line) && checked < 15) {
		...
		if (std::regex_search(line, started_detect) || std::regex_search(line, processing_detect) ||
