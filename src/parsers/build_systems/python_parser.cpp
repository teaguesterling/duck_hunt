...
bool PythonBuildParser::canParse(const std::string &content) const {
	...
	if (content.find("ERROR: Failed building wheel for") != std::string::npos ||
	    content.find("error: command") != std::string::npos &&
	        content.find("failed with exit status") != std::string::npos ||
	    content.find("setuptools") != std::string::npos && content.find(".c:") != std::string::npos) {
		return true;
	}

	// Python exception types
	if (content.find("AssertionError:") != std::string::npos || content.find("TypeError:") != std::string::npos ||
	    content.find("ValueError:") != std::string::npos || content.find("ImportError:") != std::string::npos ||
...
std::vector<ValidationEvent> PythonBuildParser::parse(const std::string &content) const {
	...
	while (std::getline(stream, line)) {
		...
		else if (line.find("error: command") != std::string::npos &&
		         line.find("failed with exit status") != std::string::npos) {
			ValidationEvent event;
			event.event_id = event_id++;
			event.tool_name = "setuptools";
			event.event_type = ValidationEventType::BUILD_ERROR;
