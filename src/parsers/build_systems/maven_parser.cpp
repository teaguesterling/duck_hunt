...
std::vector<ValidationEvent> MavenParser::parse(const std::string &content) const {
	...
	static const std::regex test_failure_pattern(
	    R"(\[ERROR\]\s+(.+?)\(\s*(.+?)\s*\)\s+Time elapsed:\s+([\d.]+)\s+s\s+<<<\s+(FAILURE|ERROR)!)");
	static const std::regex test_result_pattern(
	    R"(Tests run:\s+(\d+),\s+Failures:\s+(\d+),\s+Errors:\s+(\d+),\s+Skipped:\s+(\d+))");
	static const std::regex checkstyle_pattern(R"(\[(ERROR|WARN)\]\s+(.+?):(\d+):\s+(.+?)\s+\[(.+?)\])");
	static const std::regex spotbugs_pattern(
	    R"(\[(ERROR|WARN)\]\s+(High|Medium|Low):\s+(.+?)\s+in\s+(.+?)\s+\[(.+?)\])");
	static const std::regex pmd_pattern(R"(\[(ERROR|WARN)\]\s+(.+?):(\d+):\s+(.+?)\s+\[(.+?)\])");
	static const std::regex dependency_pattern(
	    R"(\[WARNING\]\s+(Used undeclared dependencies|Unused declared dependencies) found:)");
	...
	while (std::getline(iss, line)) {
		...
		if (std::regex_search(line, match, compile_error_pattern)) {
			...
			if (total_tests > 0) {
				...
				event.message = "Tests: " + std::to_string(total_tests) + " total, " + std::to_string(failures) +
				                " failures, " + std::to_string(errors) + " errors, " + std::to_string(skipped) +
				                " skipped";
				event.log_content = content;
				event.structured_data = "maven_build";
				events.push_back(event);
