...
bool BazelParser::canParse(const std::string &content) const {
	...
	if (content.find("INFO: Analyzed") != std::string::npos ||
	    ...
	    content.find("INFO: Found") != std::string::npos || content.find("bazel build") != std::string::npos ||
	...
	static std::regex bazel_target_pattern(
	    R"(//[a-zA-Z0-9_/.-]+:[a-zA-Z0-9_.-]+\s+\((PASSED|FAILED|TIMEOUT|SKIPPED|FLAKY))", std::regex::optimize);
...
std::vector<ValidationEvent> BazelParser::parse(const std::string &content) const {
	...
	static const std::regex bazel_analyzed(
	    R"(INFO: Analyzed (\d+) targets? \((\d+) packages? loaded, (\d+) targets? configured\)\.)");
	...
	static const std::regex bazel_test_failed(
	    R"(FAILED: (//[^/\s]+(?:/[^/\s]+)*:[^/\s]+) \(([\d\.]+)s\) \[(\d+)/(\d+) attempts\])");
	static const std::regex bazel_test_timeout(R"(TIMEOUT: (//[^/\s]+(?:/[^/\s]+)*:[^/\s]+) \(([\d\.]+)s TIMEOUT\))");
	static const std::regex bazel_test_flaky(
	    R"(FLAKY: (//[^/\s]+(?:/[^/\s]+)*:[^/\s]+) passed in (\d+) out of (\d+) attempts)");
	...
	static const std::regex bazel_analyzing(
	    R"(Analyzing: (\d+) targets? \((\d+) packages? loaded, (\d+) targets? configured\))");
	static const std::regex bazel_action_info(R"(INFO: From (.+):)");
	static const std::regex bazel_up_to_date(R"(Target (//[^/\s]+(?:/[^/\s]+)*:[^/\s]+) up-to-date:)");
	static const std::regex bazel_test_summary(
	    R"(Total: (\d+) tests?, (\d+) passed, (\d+) failed(?:, (\d+) timeout)?(?:, (\d+) flaky)?(?:, (\d+) skipped)?)");
	...
	while (reader.getLine(line)) {
		...
		if (SafeParsing::SafeRegexSearch(line, match, bazel_analyzed)) {
			...
			event.message = "Analyzed " + std::to_string(targets) + " targets (" + std::to_string(packages) +
			                " packages loaded, " + std::to_string(configured) + " targets configured)";
		...
		// Check for failed tests
			...
			event.message =
			    "Test failed (" + std::to_string(current_attempt) + "/" + std::to_string(total_attempts) + " attempts)";
			event.test_name = target;
			event.tool_name = "bazel";
			event.category = "test_result";
			event.execution_time = duration;
		...
		// Check for flaky tests
			...
			event.message = "Test is flaky - passed " + std::to_string(passed_attempts) + " out of " +
			...
			if (error_start != std::string::npos) {
				...
				if (SafeParsing::ParseFileLineColumn(error_part, file_path, line_num, col_num)) {
					...
					std::string error_msg =
					    (msg_start != std::string::npos) ? error_part.substr(msg_start + 1) : error_part;
		...
		// Check for analyzing phase
			...
			event.severity = "info";
			event.status = ValidationEventStatus::INFO;
			event.message = "Analyzing " + std::to_string(targets) + " targets (" + std::to_string(packages) +
			                " packages loaded, " + std::to_string(configured) + " targets configured)";
			event.tool_name = "bazel";
		...
		// Check for test summary
			...
			event.message = "Test Summary: " + std::to_string(total) + " tests, " + std::to_string(passed) +
			...
			event.log_content = line;
			event.structured_data =
			    "{\"total\": " + std::to_string(total) + ", \"passed\": " + std::to_string(passed) +
			    ", \"failed\": " + std::to_string(failed) + ", \"timeout\": " + std::to_string(timeout) +
			    ", \"flaky\": " + std::to_string(flaky) + ", \"skipped\": " + std::to_string(skipped) + "}";
