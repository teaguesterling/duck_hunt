...
bool ESLintJSONParser::canParse(const std::string &content) const {
	...
	if (content.find("\"filePath\"") == std::string::npos || content.find("\"messages\"") == std::string::npos) {
		return false;
	}
	...
	if (yyjson_is_arr(root)) {
		...
		if (arr_size > 0) {
			...
			if (yyjson_is_obj(first_elem)) {
				...
				is_valid = (file_path && yyjson_is_str(file_path)) && (messages && yyjson_is_arr(messages));
			}
		} else {
...
std::vector<ValidationEvent> ESLintJSONParser::parse(const std::string &content) const {
	...
	if (issue_count == 0) {
		...
		summary.message = std::to_string(issue_count) + " problem(s) in " + std::to_string(file_count) + " file(s)";
	}

	summary.structured_data =
	    "{\"files\":" + std::to_string(file_count) + ",\"problems\":" + std::to_string(issue_count) + "}";
