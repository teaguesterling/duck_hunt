...
bool TfsecJSONParser::canParse(const std::string &content) const {
	...
	if (content.find("\"results\"") != std::string::npos && content.find("\"rule_id\"") != std::string::npos &&
		...
		if (results && yyjson_is_arr(results)) {
			...
			yyjson_arr_foreach(results, idx, max, issue) {
				if (yyjson_is_obj(issue)) {
					...
					if (rule_id && yyjson_is_str(rule_id) && severity && yyjson_is_str(severity) && location &&
					    yyjson_is_obj(location)) {
						is_valid = true;
						break;
					}
				}
