...
bool StylelintJSONParser::canParse(const std::string &content) const {
	...
	if (content.find("\"source\"") != std::string::npos && content.find("\"warnings\"") != std::string::npos &&
	    content.find("\"rule\"") != std::string::npos && content.find("\"text\"") != std::string::npos) {
	...
	if (yyjson_is_arr(root)) {
		...
		yyjson_arr_foreach(root, idx, max, file_result) {
			if (yyjson_is_obj(file_result)) {
				...
				if (source && yyjson_is_str(source) && warnings && yyjson_is_arr(warnings)) {
					is_valid = true;
					break;
				}
			}
...
std::vector<ValidationEvent> StylelintJSONParser::parse(const std::string &content) const {
	...
	yyjson_arr_foreach(root, idx, max, file_result) {
		...
		// Get warnings array
		yyjson_val *warnings = yyjson_obj_get(file_result, "warnings");
		if (!warnings || !yyjson_is_arr(warnings))
			continue;
		...
		yyjson_arr_foreach(warnings, warn_idx, warn_max, warning) {
			...
			if (end_line && yyjson_is_num(end_line) && end_column && yyjson_is_num(end_column)) {
				event.suggestion = "Range: " + std::to_string(yyjson_get_int(end_line)) + ":" +
				                   std::to_string(yyjson_get_int(end_column));
			}

			// Set raw output and structured data
			event.log_content = content;
			event.structured_data = "{\"tool\": \"stylelint\", \"rule\": \"" + event.error_code +
