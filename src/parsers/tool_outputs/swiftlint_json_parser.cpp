...
bool SwiftLintJSONParser::canParse(const std::string &content) const {
	...
	if (content.find("\"file\"") != std::string::npos && content.find("\"rule_id\"") != std::string::npos &&
	    content.find("\"reason\"") != std::string::npos && content.find("\"severity\"") != std::string::npos) {
	...
	if (yyjson_is_arr(root)) {
		...
		yyjson_arr_foreach(root, idx, max, violation) {
			if (yyjson_is_obj(violation)) {
				...
				if (rule_id && yyjson_is_str(rule_id) && reason && yyjson_is_str(reason) && severity &&
				    yyjson_is_str(severity)) {
					is_valid = true;
					break;
				}
			}
