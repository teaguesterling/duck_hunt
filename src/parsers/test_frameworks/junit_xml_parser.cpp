...
bool JUnitXmlParser::canParse(const std::string& content) const {
    ...
    return HasRootElement(content, "testsuite") ||
           HasRootElement(content, "testsuites");
}
...
static void ParseTestSuiteFromJson(yyjson_val *suite, std::vector<ValidationEvent>& events,
                                    int64_t& event_id) {
