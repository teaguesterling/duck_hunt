...
void RegisterLintingToolsParsers(ParserRegistry& registry) {
    registry.registerParser(make_uniq<DelegatingParser<PylintParser>>(
        ...
        std::vector<std::string>{"pylint"}));

    registry.registerParser(make_uniq<DelegatingParser<MypyParser>>(
        "mypy_text", "Mypy Parser", ParserCategory::LINTING,
        "Python Mypy type checker output", ParserPriority::HIGH,
        std::vector<std::string>{"mypy"}));

    registry.registerParser(make_uniq<DelegatingParser<Flake8Parser>>(
        "flake8_text", "Flake8 Parser", ParserCategory::LINTING,
        "Python Flake8 style checker output", ParserPriority::HIGH,
        std::vector<std::string>{"flake8"}));

    registry.registerParser(make_uniq<DelegatingParser<BlackParser>>(
        "black_text", "Black Parser", ParserCategory::LINTING,
        "Python Black formatter output", ParserPriority::MEDIUM,
        std::vector<std::string>{"black"}));

    registry.registerParser(make_uniq<DelegatingParser<YapfTextParser>>(
        "yapf_text", "YAPF Parser", ParserCategory::LINTING,
        "Python YAPF formatter output", ParserPriority::VERY_HIGH,
        std::vector<std::string>{"yapf"}));

    registry.registerParser(make_uniq<DelegatingParser<ClangTidyParser>>(
        "clang_tidy_text", "Clang-Tidy Parser", ParserCategory::LINTING,
        "LLVM Clang-Tidy C++ linter output", ParserPriority::HIGH,
        std::vector<std::string>{"clang_tidy", "clang-tidy"}));
