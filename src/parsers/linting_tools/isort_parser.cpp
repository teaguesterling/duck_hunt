...
bool IsortParser::canParse(const std::string& content) const {
    ...
    bool has_fixing = content.find("Fixing ") != std::string::npos &&
                      content.find(".py") != std::string::npos;

    // "ERROR: isort found an import in the wrong position" - check mode
    bool has_error = content.find("ERROR: ") != std::string::npos &&
                     content.find("isort") != std::string::npos;

    // "Skipped X files" - summary
    bool has_skipped = content.find("Skipped ") != std::string::npos &&
                       content.find(" files") != std::string::npos;

    // Diff with .py files
    bool has_py_diff = content.find("--- ") != std::string::npos &&
                       content.find(".py") != std::string::npos &&
...
std::vector<ValidationEvent> IsortParser::parse(const std::string& content) const {
    ...
    while (std::getline(stream, line)) {
        ...
        if (line.find("would be reformatted") != std::string::npos ||
            ...
            event.status = line.find("would be reformatted") != std::string::npos ?
                           ValidationEventStatus::WARNING : ValidationEventStatus::INFO;
            event.message = line;
            event.ref_line = -1;
            event.ref_column = -1;
            event.tool_name = "isort";
        ...
        if (in_diff && !line.empty() && line[0] == '-' && line.size() > 1) {
            ...
            if (removed_line.find("import ") != std::string::npos ||
                removed_line.find("from ") != std::string::npos) {
                ValidationEvent event;
                event.event_id = event_id++;
                event.event_type = ValidationEventType::LINT_ISSUE;
                event.severity = "info";
        ...
        if (in_diff && !line.empty() && line[0] == '+' && line.size() > 1) {
            ...
            // Only report import lines
            if (added_line.find("import ") != std::string::npos ||
                added_line.find("from ") != std::string::npos) {
                ValidationEvent event;
                event.event_id = event_id++;
                event.event_type = ValidationEventType::LINT_ISSUE;
                event.severity = "info";
