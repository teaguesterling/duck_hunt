...
std::vector<WorkflowEvent> ParseDuckHuntWorkflowLogFromString(const std::string &content,
	...
	if (input.inputs.size() > 1) {
		...
		if (fmt == WorkflowLogFormat::UNKNOWN) {
			throw BinderException(
			    "Unknown workflow format: '" + format +
...
unique_ptr<GlobalTableFunctionState> ParseDuckHuntWorkflowLogInitGlobal(ClientContext &context,
...
TableFunctionSet GetParseDuckHuntWorkflowLogFunction() {
	...
	TableFunction single_arg("parse_duck_hunt_workflow_log", {LogicalType::VARCHAR}, ParseDuckHuntWorkflowLogFunction,
	                         ParseDuckHuntWorkflowLogBind, ParseDuckHuntWorkflowLogInitGlobal);
	...
	TableFunction two_arg("parse_duck_hunt_workflow_log", {LogicalType::VARCHAR, LogicalType::VARCHAR},
	                      ParseDuckHuntWorkflowLogFunction, ParseDuckHuntWorkflowLogBind,
	                      ParseDuckHuntWorkflowLogInitGlobal);
