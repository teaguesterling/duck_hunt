...
std::string ValidationEventStatusToString(ValidationEventStatus status) {
	...
	case ValidationEventStatus::SKIP:
		return "SKIP";
	case ValidationEventStatus::ERROR:
		return "ERROR";
	case ValidationEventStatus::WARNING:
		return "WARNING";
	case ValidationEventStatus::INFO:
		return "INFO";
	default:
		return "UNKNOWN";
	}
...
std::string ValidationEventTypeToString(ValidationEventType type) {
	switch (type) {
	case ValidationEventType::TEST_RESULT:
		return "test_result";
	case ValidationEventType::LINT_ISSUE:
		return "lint_issue";
	case ValidationEventType::TYPE_ERROR:
		return "type_error";
	case ValidationEventType::SECURITY_FINDING:
		return "security_finding";
	case ValidationEventType::BUILD_ERROR:
		return "build_error";
	case ValidationEventType::PERFORMANCE_ISSUE:
		return "performance_issue";
	case ValidationEventType::MEMORY_ERROR:
		return "memory_error";
	case ValidationEventType::MEMORY_LEAK:
		return "memory_leak";
	case ValidationEventType::THREAD_ERROR:
		return "thread_error";
	case ValidationEventType::PERFORMANCE_METRIC:
		return "performance_metric";
	case ValidationEventType::SUMMARY:
		return "summary";
	case ValidationEventType::DEBUG_EVENT:
		return "debug_event";
	case ValidationEventType::CRASH_SIGNAL:
		return "crash_signal";
...
ValidationEventStatus StringToValidationEventStatus(const std::string &str) {
	if (str == "PASS" || str == "passed")
		return ValidationEventStatus::PASS;
	if (str == "FAIL" || str == "failed")
		return ValidationEventStatus::FAIL;
	if (str == "SKIP" || str == "skipped")
		return ValidationEventStatus::SKIP;
...
ValidationEventType StringToValidationEventType(const std::string &str) {
	...
	if (str == "security_finding")
		return ValidationEventType::SECURITY_FINDING;
	if (str == "build_error")
		return ValidationEventType::BUILD_ERROR;
	if (str == "performance_issue")
		return ValidationEventType::PERFORMANCE_ISSUE;
	if (str == "memory_error")
		return ValidationEventType::MEMORY_ERROR;
	if (str == "memory_leak")
		return ValidationEventType::MEMORY_LEAK;
	if (str == "thread_error")
		return ValidationEventType::THREAD_ERROR;
	if (str == "performance_metric")
		return ValidationEventType::PERFORMANCE_METRIC;
	if (str == "summary")
		return ValidationEventType::SUMMARY;
	if (str == "debug_event")
		return ValidationEventType::DEBUG_EVENT;
	if (str == "crash_signal")
...
SeverityLevel SeverityStringToLevel(const std::string &severity_str) {
	...
	if (severity_str == "debug" || severity_str == "trace")
		return SeverityLevel::DEBUG;
	...
	if (severity_str == "error")
		return SeverityLevel::ERROR;
	if (severity_str == "critical" || severity_str == "fatal")
		return SeverityLevel::CRITICAL;
