...
std::string WorkflowEngineParser::determineSeverity(const std::string& status, const std::string& message) const {
    ...
    if (lower_status.find("fail") != std::string::npos || 
        lower_status.find("error") != std::string::npos ||
        lower_message.find("error") != std::string::npos ||
        lower_message.find("fail") != std::string::npos) {
        return "error";
    }
    
    if (lower_status.find("warn") != std::string::npos ||
        lower_message.find("warn") != std::string::npos ||
        lower_message.find("deprecated") != std::string::npos) {
        return "warning";
    }
    
    if (lower_status.find("success") != std::string::npos ||
        lower_status.find("pass") != std::string::npos ||
...
void WorkflowEngineRegistry::ensureSorted() {
    if (!sorted_) {
        std::sort(parsers_.begin(), parsers_.end(),
                 [](const std::unique_ptr<WorkflowEngineParser>& a,
                    const std::unique_ptr<WorkflowEngineParser>& b) {
