...
static EmitFilter ParseEmitFilter(const std::string &emit_str) {
	if (emit_str == "valid") {
		...
		throw BinderException("Invalid emit parameter: '%s'. Valid values are 'all', 'valid', or 'invalid'",
		                      emit_str.c_str());
	}
}
...
static void RunDiagnosis(const std::string &content, std::vector<DiagnosisEntry> &results, EmitFilter emit_filter) {
	...
	std::stable_sort(results.begin(), results.end(),
	                 [](const DiagnosisEntry &a, const DiagnosisEntry &b) { return a.priority > b.priority; });
}
...
static unique_ptr<FunctionData> DuckHuntDiagnoseParseBindFunc(ClientContext &context, TableFunctionBindInput &input,
	...
	names = {"format", "priority", "can_parse", "events_produced", "is_selected"};
...
TableFunctionSet GetDuckHuntDiagnoseParseFunction() {
	...
	TableFunction single_arg("duck_hunt_diagnose_parse", {LogicalType::VARCHAR}, DuckHuntDiagnoseFunction,
	                         DuckHuntDiagnoseParseBindFunc, DuckHuntDiagnoseInitGlobal);
	...
	TableFunction two_arg("duck_hunt_diagnose_parse", {LogicalType::VARCHAR, LogicalType::VARCHAR},
	                      DuckHuntDiagnoseFunction, DuckHuntDiagnoseParseBindFunc, DuckHuntDiagnoseInitGlobal);
...
TableFunctionSet GetDuckHuntDiagnoseReadFunction() {
	...
	TableFunction single_arg("duck_hunt_diagnose_read", {LogicalType::VARCHAR}, DuckHuntDiagnoseFunction,
	                         DuckHuntDiagnoseReadBindFunc, DuckHuntDiagnoseInitGlobal);
	set.AddFunction(single_arg);
	...
	TableFunction two_arg("duck_hunt_diagnose_read", {LogicalType::VARCHAR, LogicalType::VARCHAR},
	                      DuckHuntDiagnoseFunction, DuckHuntDiagnoseReadBindFunc, DuckHuntDiagnoseInitGlobal);
	set.AddFunction(two_arg);
